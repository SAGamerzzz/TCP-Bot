...
def run_bot_engine(bid):
    ...
    # প্রতিবার রান দিলে পুরনো ফোল্ডার ডিলিট করে নতুন ক্লোন করবে
...
<head>
    ...
    <style>
        ...
        .terminal { height: 320px; background: rgba(0, 5, 0, 0.9); border: 1px solid #0a0; margin-top: 10px; padding: 8px; font-size: 11px; overflow-y: auto; color: #0f0; white-space: pre-wrap; line-height: 1.5; }
...
def add_server():
    active_count = sum(1 for b in bots.values() if b["active"])
    if active_count < 1: # সর্বোচ্চ ১টি সার্ভার লিমিট
        for bid in ["1", "2", "3", "4"]:
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>AFRIDI HACKER PANEL</title>
    <style>
        body, html { margin: 0; padding: 0; height: 100%; background: #000; font-family: 'Courier New', monospace; overflow-x: hidden; color: #0f0; }
        canvas { position: fixed; top: 0; left: 0; z-index: -1; }
        .container { position: relative; z-index: 10; padding: 20px; }
...
<body>
    ...
    <div class="container">
        <div class="header">
            <h1 style="margin:0; font-size: 24px; letter-spacing: 5px;">AFRIDI VIP SYSTEM</h1>
            <div id="stats">CPU: 0% | RAM: 0% | ACTIVE: 0</div>
        </div>
        <div class="grid" id="card-container">
            {% for bid, b in bots.items() %}
    ...
    <script>
        ...
        function addServer() {
            fetch('/api/add_server').then(r => r.json()).then(data => {
                if(data.success) location.reload();
                else alert("MAX 4 SERVERS REACHED (LIMIT 4)");
            });
        }
...
def add_server():
    active_count = sum(1 for b in bots.values() if b["active"])
    if active_count < 4: # সর্বোচ্চ ৪টি সার্ভার লিমিট
        for bid in ["1", "2", "3", "4"]:
            if not bots[bid]["active"]:
                bots[bid]["active"] = True
                save_config()
...
# ==========================================
# 4. PREMIUM NEON GAMING UI (UPDATED HTML)
# ==========================================
HTML_PAGE = r"""
<!DOCTYPE html>
<html lang="en">
...
def add_server():
    active_count = sum(1 for b in bots.values() if b["active"])
    if active_count < 5: # সর্বোচ্চ ৫টি সার্ভার বা বোট স্লট লিমিট
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=PORT, debug=False, use_reloader=False)    print("\n"+"-"*60)
